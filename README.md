...
|max_image_width|Maximum width for images in pixels (e.g., 800). Leave empty or set to 0 to use original size. Images will maintain their aspect ratio.|False|800|
...
### Image Sizing
- By default, images are constrained to a maximum width of 800 pixels
- Images maintain their aspect ratio automatically
- Configure with `max_image_width` parameter
- Set to `0` or empty string to use original image size

Example configuration for custom image width:
```yaml
- uses: gabesw/confluence-readme-sync@v1
  with:
    max_image_width: 600  # Limit images to 600px wide
    # ... other parameters
```
## Features
